...
#Install and configure the Web Enrolment feature
...
#The templates from the MMC console are only available on the server as starting points for other templates
#from which we will issue certificates (they cannot be used directly to issue new certs)

#Duplicate the Web Server template and name it Web Intranet. Grant the Administrator user rights to enroll
...
#For a more real test create a cname for a web server eg testsite.testcorp.com
#Create a https site with server name indication with that name
#Request a cert with testsite.testcorp.com and bind it to the site
