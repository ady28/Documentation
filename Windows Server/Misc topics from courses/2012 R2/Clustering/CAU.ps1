...
#Specify the WSUS server if it is the case: http://wsus01.testcorp1.local:8530
...
#The DisableAclChecks is used here in the test environment but in production 
#you should remove it and restrict the permissions on the CAU folder
Invoke-CauRun -ClusterName FS01 -CauPluginName Microsoft.WindowsUpdatePlugin, Microsoft.HotfixPlugin -CauPluginArguments @{'QueryString'="IsInstalled=0 and IsHidden=0"},@{'HotfixRootFolderPath' = '\\bmc01\CAU$';'DisableAclChecks' = 'True'} -Force
#Prod: Invoke-CauRun -ClusterName FS01 -CauPluginName Microsoft.WindowsUpdatePlugin, Microsoft.HotfixPlugin -CauPluginArguments @{'QueryString'="IsInstalled=0 and IsHidden=0"},@{'HotfixRootFolderPath' = '\\bmc01\CAU$'} -Force
